...
Results_write_path = "Finally_pred.txt"
Results_write_path_entity = "Finally_pred_entity.txt"
Results_write_path_context = "Finally_pred_context.txt"
...
####### ACL 2020 temp test
Seg_path_test = "./deNER/ltp.simplify.ner.test.mask.j.txt"
...
def only_evaluate_F(config, model, data_iter, test=False):
    ...
    fw_entity = open(Results_write_path_entity, "w", encoding="utf-8")
    fw_context = open(Results_write_path_context, "w", encoding="utf-8")
    ...
    with torch.no_grad():
        ...
        for (test_entity, test_context, labels) in data_iter:
            ...
            outputs, outputs_entity, outputs_context = model(test_entity, test_context)
            loss = torch.nn.functional.cross_entropy(outputs, labels)
            loss_total += loss
            labels = labels.data.cpu().numpy()
            predic = torch.max(outputs.data, 1)[1].cpu().numpy()
    ...
    for id_label in predict_all_entity:
        fw.write(str(id_to_label[id_label]))
        fw.write('\n')
    fw.close()

    for id_label in predict_all_context:
        fw.write(str(id_to_label[id_label]))
        fw.write('\n')
    fw.close()
